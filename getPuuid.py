	...
	def write_data(self, newRecords):
		...
		if len(newRecords)>0:
			for idx,r in enumerate(newRecords):
				...
				updateQuery = f"UPDATE summoners SET puuid='{r['puuid']}', accountid='{r['accountid']}', last_updated='{now}' WHERE summonerid='{r['summonerid']}' and region='{self.region}'"
				self.logger.debug(updateQuery)
				err = self.execute(updateQuery, commit=False)
				self.msg = str(f"W{idx}")
				if err:
					self.logger.error(f"something went wrong writing: {updateQuery}")
				else:
					updateCount += 1
				self.commit()
...
try:
	while not quitEvent.is_set():
		...
		msg+=f"{str(numRunningThreads):^{len('running threads')}}"
		print(msg,end="")
		time.sleep(1)
